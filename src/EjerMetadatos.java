...
public class EjerMetadatos {
    ...
    // exercicio 9c
    // Para todas as táboas de bases de datos ADD obtén: o nome das táboa e o tipo
    // de táboa.
    ...
    public void comb(String bdconnect, String infobd) {
        abrirConexion(bdconnect, "localhost", "root", "");
        String bd;
        try {
            dbmt = this.conexion.getMetaData();
            ResultSet rs = dbmt.getCatalogs();
    ...
    public void infoConsulta(String bd, String query) {
        abrirConexion(bd, "localhost", "root", "");
        try (Statement sta = this.conexion.createStatement()) {
            ...
            for (int i = 1; i <= md.getColumnCount(); i++) {
                System.out.printf("Nombre: %s\tAlias: %s\tTipo dato: %s\tAutoincrement: %b\tNull: %s\n",
                        md.getColumnName(i), md.getColumnLabel(i), md.getColumnTypeName(i), md.isAutoIncrement(i),
                        md.isNullable(i) == 1 ? "True" : "False");
            }
        } catch (SQLException e) {
            System.out.printf("Error: (%d) %s", e.getErrorCode(), e.getLocalizedMessage());
        }
